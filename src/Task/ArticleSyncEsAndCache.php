...
/**
 * 间隔一段时间晨，定时执行 视频同步到ES和Cache
 *
 * @BeTask("视频境量同步到ES和Cache", schedule="20 * * * *")
 */
class VideoSyncEsAndCache extends TaskInterval
{
    ...
    public function execute()
    {
        ...
        $db = Be::getDb();
        $sql = 'SELECT * FROM video_video WHERE is_enable != -1 AND update_time >= ? AND update_time <= ?';
        ...
        $db = Be::getDb();
        $sql = 'SELECT * FROM video_video_comment WHERE is_enable != -1 AND update_time >= ? AND update_time < ?';
        $objs = $db->getYieldObjects($sql, [$d1, $d2]);
