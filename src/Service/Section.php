...
class Section
{
     ...
     * @return string
     */
    public function makeVideosSection(object $section, string $class, array $videos, string $defaultMoreLink = null): string
    {
        $html = '';
        ...
        $html .= '#' . $section->id . ' .' . $class . ' .video-image {';
        $html .= 'width: 100%;';
        $html .= '}';

        $html .= '#' . $section->id . ' .' . $class . ' .video-image img {';
        $html .= 'max-width: 100%;';
        $html .= '}';
        ...
        if (count($videos) === 0) {
            ...
            $html .= beLang('App.Video', 'VIDEO.NO_MATCH_RESULT');
            $html .= '</div>';
            $html .= '</div>';
        } else {
            ...
            foreach ($videos as $video) {
                ...
                $html .= '<div class="video-image">';
                $html .= '<a class="be-d-inline-block" href="';
                $html .= beUrl('Video.Video.detail', ['id'=> $video->id]);
                $html .= '" title="';
                $html .= $video->title;
                $html .= '"';
                if (!$isMobile) {
                ...
                $html .= '<img src="';
                if ($video->image === '') {
                    $html .= \Be\Be::getProperty('App.Video')->getWwwUrl() . '/video/images/no-image-m.jpg';
                } else {
                    $html .= $video->image;
                }
                $html .= '" alt="';
                ...
                $html .= '<span>';
                $html .= date(beLang('App.Video', 'VIDEO.PUBLISH_TIME_YYYY_MM_DD'), strtotime($video->publish_time));
                $html .= '</span>';
                if ($video->author !== '') {
                    $html .= '<span class="be-ml-100">' . beLang('App.Video', 'VIDEO.AUTHOR') . ': ' . $video->author . '</span>';
                }
                $html .= '</div>';
     ...
     * @return string
     */
    public function makePagedVideosSection(object $section, string $class, array $result): string
    {
        $html = '';
        $html .= '<style type="text/css">';
        $html .= $section->getCssBackgroundColor($class);
        ...
        if ($pages > 1) {
            ...
            if ($page > 1) {
                $params['page'] = $page - 1;
                $html .= '<a href="' . beUrl($route, $params) . '">' . beLang('App.Video', 'PAGINATION.PREVIOUS'). '</a>';
            } else {
            ...
            if ($page < $pages) {
                $params['page'] = $page + 1;
                $html .= '<a href="' . beUrl($route, $params) . '">' . beLang('App.Video', 'PAGINATION.NEXT'). '</a>';
            } else {
     ...
     * @return string
     */
    public function makeSideVideosSection(object $section, string $class, array $videos, string $defaultMoreLink = null): string
    {
        $html = '';
        $html .= '<style type="text/css">';
        $html .= $section->getCssBackgroundColor($class);
        ...
        foreach ($videos as $video) {
            ...
            $html .= '<a class="be-d-block be-t-ellipsis" href="' . beUrl('Video.Video.detail', ['id' => $video->id]) . '" title="' . $video->title . '"';
            if (!$isMobile) {
                $html .= ' target="_blank"';
            }
            $html .= '>';
