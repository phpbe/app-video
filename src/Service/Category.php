...
class Category
{
    ...
    public function getCategories(int $n = 0): array
    {
        ...
        $key = 'App:Video:Categories';
        $categories = $cache->get($key);

        if (!$categories) {
            $table =  Be::getTable('video_category');
    ...
    {
        $cache = Be::getCache();
        $key = 'App:Video:Category:' . $categoryId;
        ...
        if ($category === '-1') {
            throw new ServiceException(beLang('App.Video', 'CATEGORY.NOT_EXIST'));
        }
