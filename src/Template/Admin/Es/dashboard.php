    ...
    <div class="be-bc-fff be-p-150" id="app" v-cloak>
        ...
        if (!$this->configEs->enable) {
            ...
            <div class="be-fw-bold">
                ES搜索引擎未启用！
                <el-link class="be-ml-100" type="primary" href="<?php echo beAdminUrl('Video.Config.dashboard', ['configName' => 'Es']); ?>">修改</el-link>
            </div>
            <?php
        } else {
        ...
        if (count($formDatas) > 0) {
            ...
            <ul>
                ...
                <li class="be-c-999">参考：<el-link type="primary" href="https://www.liu12.com/video/es-number-of-rshards-and-eplicas" target="_blank">https://www.liu12.com/video/es-number-of-rshards-and-eplicas</el-link></li>
            </ul>
            <?php
        }
        ?>
            ...
            methods: {
                createIndex: function (formRef, formData) {
                    ...
                    this.$refs[formRef].validate(function (valid) {
                        if (valid) {
                            _this.loading = true;
                            _this.$http.post("<?php echo beAdminUrl('Video.Es.createIndex'); ?>", {
                ...
                deleteIndex: function (indexname) {
                    ...
                    _this.$confirm("本操作将删除索引中的所有数据，确认要岫除么？", "删除确认", {
                        ...
                        _this.loading = true;
                        _this.$http.post("<?php echo beAdminUrl('Video.Es.deleteIndex'); ?>", {
                            formData: {
                                name: indexname
                            }
                        }).then(function (response) {
